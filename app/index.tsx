...
function Status({ value, retry }: { value: ReturnType<typeof useMatchDatabase>['status']; retry: () => Promise<unknown> }) {
  ...
  const text = value.phase === 'loading' ? '記録を読み込み中…' : value.phase === 'saving' ? '保存中…' : value.phase === 'saved' ? (value.migrated ? '旧記録を移行して保存済み' : '端末内に保存済み') : value.phase === 'load-error' ? `復元失敗：${value.message}` : `未保存：${value.message}`;
