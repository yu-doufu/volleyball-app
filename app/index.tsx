...
import {
  aggregateMatchStats, clearActiveMatch, clearCurrentSet, completeCurrentSet, completeMatch, deriveSet,
...
type Screen = 'home' | 'setup' | 'live' | 'clear-record' | 'finish-set' | 'after-set' | 'finish-match' | 'history' | 'detail' | 'set-stats';
...
export default function AppScreen() {
  ...
  return <SafeAreaView style={styles.safe}><Status retry={store.retry} value={store.status}/>{body}{screen === 'live' && active && currentSet && <View style={styles.clearControl}><Button danger disabled={!canOperate} onPress={() => { setClearTarget(null); setScreen('clear-record'); }} title="記録をクリア"/></View>}</SafeAreaView>;
}
