...
export default function AppScreen() {
  ...
  else if (screen === 'live' && active && currentSet && current) { const canReturnEmpty = canRemoveCurrentEmptySet(database); const canReturnWithDiscard = canDiscardCurrentNewSet(database); body = <View style={styles.flex}><View style={styles.liveHead}><Text style={styles.setTitle}>第{currentSet.number}セット</Text><Text style={styles.liveSubtitle}>現在のスコア</Text><ScoreControls homeName={active.homeTeam || '自チーム未設定'} awayName={active.awayTeam || '相手未設定'} homeScore={current.homeScore} awayScore={current.awayScore} disabled={!canOperate} onChange={(side, value) => update(db => setScore(db, side, value))}/></View><ScrollView contentContainerStyle={styles.stats} keyboardDismissMode="on-drag" keyboardShouldPersistTaps="handled"><Text style={styles.panelHeading}>このセットのスタッツ</Text><StatsPanel highlightResetKey={`${active.id}:${currentSet.id}:${undoRevision}`} counts={current.stats.counts} disabled={!canOperate} onRecord={input => update(db => recordStat(db, input))}/><View style={styles.secondaryActions}><Pressable accessibilityRole="button" disabled={!canOperate} onPress={() => { setClearTarget(null); setScreen('clear-record'); }} style={({ pressed }) => [styles.clearLink, !canOperate && styles.disabled, pressed && styles.pressed]}><Text style={styles.clearLinkText}>記録をクリア</Text></Pressable>{canReturnEmpty && <Button disabled={!canOperate} onPress={() => void returnToAfterSet(false)} title="前のセットへ戻る"/>}{!canReturnEmpty && canReturnWithDiscard && <Button disabled={!canOperate} onPress={() => { setMessage(''); setScreen('return-to-after-set'); }} title="前のセットへ戻る"/>}</View><View style={styles.primaryAction}><Button onPress={startEndSet} title="セット終了"/></View></ScrollView><View style={styles.undo}><View style={styles.undoCopy}><Text style={styles.undoLabel}>Undo</Text><Text numberOfLines={2} style={styles.last}>{operationText(current.lastOperation)}</Text></View><View style={styles.undoButton}><Button disabled={!current.lastOperation || !canOperate} onPress={() => { setUndoRevision(value => value + 1); update(undoCurrentSet); }} title="1つ戻す"/></View></View></View>; }
...
const styles = StyleSheet.create({
  ...
  card:{backgroundColor:'#fff',borderColor:'#d9e0e3',borderRadius:10,borderWidth:1,gap:8,padding:12},cardTitle:{color:'#172126',fontSize:18,fontWeight:'800'},warning:{color:'#8b3c24',fontSize:14,fontWeight:'700'},warningBox:{backgroundColor:'#fff2df',borderRadius:8,gap:8,padding:10},liveHead:{backgroundColor:'#e8eef1',paddingHorizontal:10,paddingTop:8},setTitle:{color:'#172126',fontSize:20,fontWeight:'800',textAlign:'center'},liveSubtitle:{color:'#53636a',fontSize:12,fontWeight:'700',marginBottom:2,textAlign:'center'},stats:{gap:8,padding:10,paddingBottom:18},panelHeading:{color:'#263238',fontSize:18,fontWeight:'800',marginTop:2},secondaryActions:{backgroundColor:'#fff',borderColor:'#d9e0e3',borderRadius:10,borderWidth:1,gap:4,padding:6},primaryAction:{marginTop:2},undo:{alignItems:'center',backgroundColor:'#fff',borderTopColor:'#d9e0e3',borderTopWidth:1,flexDirection:'row',gap:8,padding:8},undoCopy:{flex:1,minWidth:0},undoLabel:{color:'#172126',fontSize:13,fontWeight:'800'},undoButton:{width:124},last:{color:'#344249',fontSize:13,fontWeight:'700',marginBottom:0},clearLink:{alignItems:'center',alignSelf:'center',justifyContent:'center',minHeight:36,paddingHorizontal:12},clearLinkText:{color:'#8c3025',fontSize:14,fontWeight:'800',textDecorationLine:'underline'},
