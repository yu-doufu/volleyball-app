...
import {
  aggregateMatchStats, canDiscardCurrentNewSet, canRemoveCurrentEmptySet, clearActiveMatch, clearCurrentSet,
  completeCurrentSet, completeMatch, deriveCompletedMatchSummary, deriveSet, discardCurrentNewSet,
  editCompletedSetScore, getActiveMatch, getCompletedMatchesNewestFirst, getCurrentSet, recordStat,
  removeCurrentEmptySet, setScore, startMatch, startNextSet, undoCurrentSet, updateMatchInfo,
...
type Screen = 'home' | 'setup' | 'live' | 'return-to-after-set' | 'clear-record' | 'finish-set' | 'after-set' | 'finish-match' | 'history' | 'detail' | 'set-stats';
...
function Status({ value, retry }: { value: ReturnType<typeof useMatchDatabase>['status']; retry: () => Promise<unknown> }) {
  ...
  if (value.phase === 'saved') return null;
  const text = value.phase === 'loading' ? '記録を読み込み中…' : value.phase === 'saving' ? '保存中…（完了表示まで閉じないでください）' : value.phase === 'load-error' ? `復元失敗：${value.message}` : `未保存：${value.message}`;
...
export default function AppScreen() {
  ...
  const [clearTarget, setClearTarget] = useState<'set' | 'match' | null>(null);
  ...
  const returnToAfterSet = async (discard: boolean) => {
    const ok = await updateAndWait(discard ? discardCurrentNewSet : removeCurrentEmptySet);
    if (ok) {
      setMessage('');
      setScreen('after-set');
    } else {
      setMessage('保存に失敗したため、前のセットへは戻っていません。再試行してください。');
      setScreen('live');
    }
  };
  ...
  const executeClear = () => {
    if (clearTarget === 'set') {
      ...
      update(clearCurrentSet);
      setClearTarget(null);
      setScreen('live');
    } else if (clearTarget === 'match') {
      update(clearActiveMatch);
      setClearTarget(null);
      setMatchId(null);
      setSetId(null);
      setScreen('home');
  ...
  if (screen === 'clear-record' && active && currentSet) {
    const confirmation = clearTarget !== null;
    return <SafeAreaView style={styles.safe}><Status retry={store.retry} value={store.status}/><ScrollView contentContainerStyle={styles.page}><Text style={styles.title}>記録をクリア</Text>{!confirmation ? <><Text style={styles.warning}>通常のスタッツ入力とは離れた場所で、クリアする範囲を選択します。</Text><Button danger disabled={!canOperate} onPress={() => setClearTarget('set')} title="現在のセットをクリア"/><Button danger disabled={!canOperate} onPress={() => setClearTarget('match')} title="記録中の試合全体をクリア"/><Button onPress={() => setScreen('live')} title="記録へ戻る"/></> : <View style={styles.warningBox}><Text style={styles.section}>{clearTarget === 'set' ? '現在のセットをクリアしますか？' : '記録中の試合全体をクリアしますか？'}</Text><Text style={styles.warning}>{clearTarget === 'set' ? 'このセットの点数、スタッツ、直前入力、Undo履歴が削除されます。確定済みセットと試合情報は残ります。' : '記録中の試合とその全セットが削除されます。終了済みの過去試合と自チーム名の設定は残ります。'}</Text><Button danger disabled={!canOperate} onPress={executeClear} title="削除してクリア"/><Button onPress={() => setClearTarget(null)} title="キャンセル"/></View>}</ScrollView></SafeAreaView>;
  }

  if (screen === 'return-to-after-set' && active && currentSet) {
    return <SafeAreaView style={styles.safe}><Status retry={store.retry} value={store.status}/><ScrollView contentContainerStyle={styles.page}><Text style={styles.title}>前のセットへ戻る</Text><Text style={styles.warning}>この新しく開始したセットには入力があります。このセットだけを破棄して、前に確定したセットへ戻ります。前のセットの点数・スタッツ・記録は残ります。</Text><Text style={styles.error}>{message}</Text><Button danger disabled={!canOperate} onPress={() => void returnToAfterSet(true)} title="新しいセットを破棄して戻る"/><Button onPress={() => { setMessage(''); setScreen('live'); }} title="キャンセルして記録を続ける"/></ScrollView></SafeAreaView>;
  }
  ...
  else if (screen === 'live' && active && currentSet && current) { const canReturnEmpty = canRemoveCurrentEmptySet(database); const canReturnWithDiscard = canDiscardCurrentNewSet(database); body = <View style={styles.liveViewport}><View style={styles.live}><View style={styles.liveHead}><View style={styles.liveTitleRow}><Text style={styles.setTitle}>第{currentSet.number}セット</Text><Pressable accessibilityRole="button" disabled={!canOperate} onPress={() => { setClearTarget(null); setScreen('clear-record'); }} style={({ pressed }) => [styles.clearButton, !canOperate && styles.disabled, pressed && styles.pressed]}><Text style={styles.clearButtonText}>記録をクリア</Text></Pressable></View><View pointerEvents="none" style={styles.teamNames}><Text ellipsizeMode="head" numberOfLines={1} style={styles.teamName}>{active.homeTeam || '自チーム未設定'}</Text><Text ellipsizeMode="tail" numberOfLines={1} style={styles.teamName}>{active.awayTeam || '相手未設定'}</Text></View><ScoreControls hideNames homeName={active.homeTeam || '自チーム未設定'} awayName={active.awayTeam || '相手未設定'} homeScore={current.homeScore} awayScore={current.awayScore} disabled={!canOperate} onChange={(side, value) => update(db => setScore(db, side, value))}/></View><ScrollView contentContainerStyle={styles.stats} keyboardDismissMode="on-drag" keyboardShouldPersistTaps="handled"><StatsPanel highlightResetKey={`${active.id}:${currentSet.id}:${undoRevision}`} counts={current.stats.counts} disabled={!canOperate} onRecord={input => update(db => recordStat(db, input))}/><View style={styles.primaryAction}><Button onPress={startEndSet} title="セット終了"/></View>{canReturnEmpty && <Pressable accessibilityRole="button" disabled={!canOperate} onPress={() => void returnToAfterSet(false)} style={({ pressed }) => [styles.returnLink, !canOperate && styles.disabled, pressed && styles.pressed]}><Text style={styles.returnLinkText}>前のセットへ戻る</Text></Pressable>}{!canReturnEmpty && canReturnWithDiscard && <Pressable accessibilityRole="button" disabled={!canOperate} onPress={() => { setMessage(''); setScreen('return-to-after-set'); }} style={({ pressed }) => [styles.returnLink, !canOperate && styles.disabled, pressed && styles.pressed]}><Text style={styles.returnLinkText}>前のセットへ戻る</Text></Pressable>}</ScrollView><View style={styles.undo}><View style={styles.undoCopy}><Text style={styles.undoLabel}>直前の入力</Text><Text numberOfLines={2} style={styles.last}>{operationText(current.lastOperation)}</Text></View><View style={styles.undoButton}><Button disabled={!current.lastOperation || !canOperate} onPress={() => { setUndoRevision(value => value + 1); update(undoCurrentSet); }} title="1つ戻す"/></View></View></View></View>; }
  ...
  else if (screen === 'history') { const done = getCompletedMatchesNewestFirst(database.matches); body = <ScrollView contentContainerStyle={styles.page}><Text style={styles.title}>過去試合</Text>{done.map(m => { const summary = deriveCompletedMatchSummary(m); return <Pressable key={m.id} onPress={() => { setMatchId(m.id); setScreen('detail'); }} style={styles.match}><Text style={styles.cardTitle}>{m.date || '日付未設定'}</Text><Text>{m.homeTeam || '自チーム未設定'} − {m.awayTeam || '相手未設定'}</Text><Text style={styles.historyScore}>セット勝数　{summary.homeSetWins} − {summary.awaySetWins}</Text><Text style={styles.historySets}>{summary.sets.length > 0 ? summary.sets.map(set => `第${set.number}セット ${set.homeScore ?? '—'}-${set.awayScore ?? '—'}`).join(' / ') : '確定済みセットの点数はありません'}</Text></Pressable>; })}{done.length === 0 && <Text>終了した試合はありません。</Text>}<Button onPress={() => setScreen('home')} title="ホームへ"/></ScrollView>; }
...
const styles = StyleSheet.create({
  safe:{backgroundColor:'#f5f7f8',flex:1},flex:{flex:1},center:{flex:1,justifyContent:'center',padding:16},page:{gap:12,padding:14,paddingBottom:30},liveViewport:{alignItems:'center',flex:1},live:{flex:1,maxWidth:720,width:'100%'},title:{color:'#172126',fontSize:24,fontWeight:'800'},section:{color:'#263238',fontSize:19,fontWeight:'800',marginTop:8},label:{color:'#263238',fontSize:16,fontWeight:'800'},input:{backgroundColor:'#fff',borderColor:'#788991',borderRadius:8,borderWidth:1,color:'#111',fontSize:18,minHeight:52,paddingHorizontal:10},
  ...
  card:{backgroundColor:'#fff',borderColor:'#d9e0e3',borderRadius:10,borderWidth:1,gap:8,padding:12},cardTitle:{color:'#172126',fontSize:18,fontWeight:'800'},warning:{color:'#8b3c24',fontSize:14,fontWeight:'700'},warningBox:{backgroundColor:'#fff2df',borderRadius:8,gap:8,padding:10},liveHead:{backgroundColor:'#fbfdff',paddingHorizontal:16,paddingTop:14,paddingBottom:12},liveTitleRow:{alignItems:'center',flexDirection:'row',justifyContent:'space-between',marginBottom:10},teamNames:{flexDirection:'row',gap:12,paddingHorizontal:16},teamName:{color:'#102a43',flex:1,fontSize:15,fontWeight:'800',minWidth:0,textAlign:'center'},setTitle:{color:'#102a43',fontSize:28,fontWeight:'800'},stats:{gap:14,padding:16,paddingBottom:24},primaryAction:{marginTop:2},undo:{alignItems:'center',backgroundColor:'#fff',borderColor:'#e1ebf2',borderTopWidth:1,flexDirection:'row',gap:8,padding:12},undoCopy:{flex:1,minWidth:0},undoLabel:{color:'#526b80',fontSize:13,fontWeight:'800'},undoButton:{width:124},last:{color:'#163a57',fontSize:15,fontWeight:'700',marginBottom:0},clearButton:{alignItems:'center',borderColor:'#b3261e',borderRadius:10,borderWidth:1,justifyContent:'center',minHeight:44,paddingHorizontal:12},clearButtonText:{color:'#a61b16',fontSize:14,fontWeight:'800'},returnLink:{alignItems:'center',alignSelf:'center',justifyContent:'center',minHeight:44,paddingHorizontal:12},returnLinkText:{color:'#175d8a',fontSize:14,fontWeight:'800',textDecorationLine:'underline'},
  match:{backgroundColor:'#fff',borderColor:'#d9e0e3',borderRadius:9,borderWidth:1,gap:6,minHeight:64,padding:12},historyScore:{color:'#172126',fontSize:16,fontWeight:'800'},historySets:{color:'#344249',fontSize:14,fontWeight:'700',lineHeight:20},link:{alignItems:'center',justifyContent:'center',minHeight:44},linkText:{color:'#145a86',fontSize:15,fontWeight:'800',textDecorationLine:'underline'},edit:{alignItems:'center',backgroundColor:'#fff',borderRadius:8,flexDirection:'row',flexWrap:'wrap',gap:6,padding:8},editLabel:{fontSize:13,fontWeight:'800',width:70},smallInput:{borderColor:'#788991',borderRadius:6,borderWidth:1,fontSize:18,minHeight:42,textAlign:'center',width:52},smallButton:{backgroundColor:'#174e78',borderRadius:6,justifyContent:'center',minHeight:42,paddingHorizontal:10},editMsg:{color:'#356447',fontSize:11,width:'100%'},
});
